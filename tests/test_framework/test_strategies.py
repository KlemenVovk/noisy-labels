...
@pytest.mark.parametrize(
    "config, target_dict", [(k, v) for k, v in reprod_targets.items()]
)
def test_reprod_perf(
    config: Type[MethodConfig], target_dict: dict[str, tuple[Target]], capsys
):
    with (
        capsys.disabled()
    ):  # disable stdout capturing - can be removed when the test works
