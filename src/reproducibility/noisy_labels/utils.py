...
def update_config(base_config, new_data_config, new_seed, logdir="../logs"):
    class UpdatedConfig(base_config):
        data_config = (
            new_data_config
            if base_config._data_config_wrapper is None
            else base_config._data_config_wrapper(new_data_config)
        )
        seed = new_seed
        trainer_args = {
            **base_config.trainer_args,
            "logger": CSVLogger(
                f"{logdir}/{new_data_config.__name__}",
                name=f"{base_config.__name__}_{new_seed}",
            ),
        }
...
class TestCallback(L.Callback):
    ...
    def on_train_epoch_end(
        ...
        for batch in trainer.datamodule.test_dataloader()[0]:
            batch = [
                batch[0].to(trainer.model.device),
                batch[1].to(trainer.model.device),
            ]
