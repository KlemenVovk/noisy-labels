...
def noisify_instance(x, y, noise_rate):
    ...
    instance_flip_rates = torch.nn.init.trunc_normal_(
        torch.empty(n_samples), mean=noise_rate, std=0.1, a=0, b=1
    )  # [B]

    # Probability that instance's label is flipped to each label (not really a probability)
    instance_label_noise = torch.normal(
        0, 1, size=(feature_dim, num_classes)
    )  # [C*H*W, CLS]

    p = x @ instance_label_noise  # [B, FEAT] @ [FEAT, CLS] -> [B, CLS]
    p[np.arange(p.shape[0]), y] = -torch.inf  # mask softmax
    p = instance_flip_rates.unsqueeze(-1) * F.softmax(
        p, dim=-1
    )  # [B, 1] * [B, CLS] -> [B, CLS]
    p[np.arange(p.shape[0]), y] = (
        1 - instance_flip_rates
    )  # this is actually wrong since p[true_class] will not be 1 - instance_flip_rates after normalization
    p /= p.sum(axis=1, keepdim=True)  # normalise rows
    y_noise = (
        torch.multinomial(p, 1).flatten()
    )  # batch sample multinomial distributions (rows of p are individual multinomials)
...
def generate_instance_dependent_noise(
    x: torch.Tensor, y: torch.Tensor, noise_rate: float, num_classes: int
) -> torch.Tensor:
