...
from noisypy.configs.data.cifar10 import (
    CIFAR10,
...
class cifar10_DISC_repro(DataConfig):
    ...
    dataset_train_augmentation = Compose(
        [
            AddNoise(
                InstanceNoise(
                    torch.load(
                        "src/reproducibility/learning_strategies/DISC/assets/original_sym20_noise.pt",
                        weights_only=False,
                    )
                )
            ),
    ...
    dataset_val_args = dict(train=False, transform=cifar10_test_transform)

    dataset_test_cls = CIFAR10
    dataset_test_args = dict(train=False, transform=cifar10_test_transform)

    datamodule_args = dict(batch_size=128, num_workers=4)
