...
from utils.divide_mix import ResNet18, NegEntropy, SemiLoss


class DivideMix(L.LightningModule):
    def __init__(self, initial_lr, momentum, weight_decay, warmup_epochs, noise_type, p_thresh, temperature, alpha, datamodule):
        super().__init__()
        ...
        self.model1 = ResNet18(num_classes=self.num_classes)
        self.model2 = ResNet18(num_classes=self.num_classes)
    ...
    def on_train_epoch_start(self):
        if self.current_epoch >= self.hparams.warmup_epochs:
            ...
            self.trainer.datamodule.set_probabilities(prob2, prob1)
            self.trainer.datamodule.set_predictions(pred2, pred1)


    def on_train_epoch_end(self) -> None:
        if self.current_epoch + 1 == self.hparams.warmup_epochs:
            self.trainer.datamodule.end_warmup()
    ...
    def eval_train(self, model: torch.nn.Module, loss_hist: list, data_loader: DataLoader, i: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            for batch in tqdm(data_loader['eval_train'], desc=f'Label Guessing {i}'):
        ...
        if self.trainer.datamodule.train_dataset1.noise_rate == 0.9:
    ...
    def training_step(self, batch: Any, batch_idx: int) -> STEP_OUTPUT:
        ...
        if self.current_epoch < self.hparams.warmup_epochs:
            batch1 = batch['warmup1']
            batch2 = batch['warmup2']
            loss1 = self.warmup_step(batch1, self.model1, opt1, batch_idx)
            loss2 = self.warmup_step(batch2, self.model2, opt2, batch_idx)
        else:
            labeled_batch1 = batch['labeled1']
            unlabeled_batch1 = batch['unlabeled1']
            labeled_batch2 = batch['labeled2']
            unlabeled_batch2 = batch['unlabeled2']
            num_iter1 = len(self.trainer.datamodule.train_dataset1) // self.trainer.datamodule.batch_size
            num_iter2 = len(self.trainer.datamodule.train_dataset2) // self.trainer.datamodule.batch_size
    ...
    def configure_optimizers(self):
        optimizer1 = torch.optim.SGD(self.model1.parameters(), lr=self.hparams.initial_lr, momentum=self.hparams.momentum, weight_decay=self.hparams.weight_decay)
        optimizer2 = torch.optim.SGD(self.model2.parameters(), lr=self.hparams.initial_lr, momentum=self.hparams.momentum, weight_decay=self.hparams.weight_decay)
        # if epoch >= 150 : lr /= 10
        lr_lambda = lambda epoch: 0.1 if epoch >= 150 else 1
