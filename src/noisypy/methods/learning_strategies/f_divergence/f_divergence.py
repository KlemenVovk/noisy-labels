...
class FDivergence(LearningStrategyWithWarmupModule):
    ...
    def fdivergence_training_step(
        self, batch: Any, batch_idx: int, *args
    ) -> STEP_OUTPUT:
    ...
    def fdivergence_validation_step(
        self, batch: Any, batch_idx: int, *args
    ) -> STEP_OUTPUT:
    ...
    def validation_step(self, batch: Any, batch_idx: int, *args) -> STEP_OUTPUT:
        if self.current_epoch <= self.warmup_epochs:
            ...
            self.log("val_loss_fdiv", 100.0, prog_bar=True)
            # also log the combined curve to compare at the end
            self.log("val_loss", warmup_loss, prog_bar=True)

        else:
            f_div_loss, val_acc, f_score = self.fdivergence_validation_step(
            ...
            self.log("val_loss_warmup", 100.0, prog_bar=True)
            # also log the combined curve to compare at the end
    ...
    def configure_optimizers(self):
        optim_warmup = self.optimizer_cls[0](
            params=self.model.parameters(), **self.optimizer_args[0]
        )
        optim_fdivergence = self.optimizer_cls[1](
            params=self.model.parameters(), **self.optimizer_args[1]
        )

        scheduler_warmup = self.scheduler_cls[0](optim_warmup, **self.scheduler_args[0])
        scheduler_fdivergence = self.scheduler_cls[1](
            optim_fdivergence, **self.scheduler_args[1]
        )

        return [optim_warmup, optim_fdivergence], [
            scheduler_warmup,
            scheduler_fdivergence,
        ]
