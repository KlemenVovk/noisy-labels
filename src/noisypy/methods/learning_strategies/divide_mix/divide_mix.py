...
class DivideMix(LearningStrategyModule):
    ...
    def on_train_epoch_start(self) -> None:
        if self.current_epoch >= self.warmup_epochs:
            ...
            prob1, self.loss_hist1 = self.eval_train(
                self.model1, self.loss_hist1, data_loader, 1
            )
            prob2, self.loss_hist2 = self.eval_train(
                self.model2, self.loss_hist2, data_loader, 2
            )
    ...
    def warmup_step(
        self, batch: Any, model: Module, opt: Optimizer, batch_idx: int
    ) -> torch.Tensor:
    ...
    def eval_train(
        self, model: Module, loss_hist: list, data_loader: DataLoader, i: int
    ) -> torch.Tensor:
    ...
    def train_step(
        ...
        with torch.no_grad():
            ...
            pu = (
                softmax(outputs_u11, dim=1)
                + softmax(outputs_u12, dim=1)
        ...
        Lx, Lu, lamb = self.SemiLoss(
            logits_x,
            mixed_target[: batch_size * 2],
            logits_u,
            mixed_target[batch_size * 2 :],
            self.current_epoch + batch_idx / num_iter,
            self.warmup_epochs,
            self.lambda_u,
        )
    ...
    def training_step(self, batch: Any, batch_idx: int) -> STEP_OUTPUT:
        ...
        if self.current_epoch < self.warmup_epochs:
            ...
            if (
                unlabeled_batch1[0].size(0) <= 1
                or unlabeled_batch2[0].size(0) <= 1
                or labeled_batch1[0].size(0) <= 1
                or labeled_batch2[0].size(0) <= 1
            ):
                return  # FIXME: Should probably drop last batch in the dataloader.
            num_iter1 = (
                len(self.trainer.datamodule.train_datasets[BATCH_MAP["labeled1"]])
                // self.trainer.datamodule.batch_size
            )
            num_iter2 = (
                len(self.trainer.datamodule.train_datasets[BATCH_MAP["labeled2"]])
                // self.trainer.datamodule.batch_size
            )
