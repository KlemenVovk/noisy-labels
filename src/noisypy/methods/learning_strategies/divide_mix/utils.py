...
def set_predictions(datamodule, pred1, pred2):
    ...
    datamodule.train_datasets[
        BATCH_MAP["labeled1"]
    ].unlabeled_indices = unlabeled_indices1
    ...
    datamodule.train_datasets[
        BATCH_MAP["unlabeled1"]
    ].unlabeled_indices = unlabeled_indices1
    datamodule.train_datasets[
        BATCH_MAP["unlabeled2"]
    ].unlabeled_indices = unlabeled_indices2


def linear_rampup(current, warm_up, rampup_length=16, lambda_u=25):
    current = torch.clip(
        torch.tensor(current - warm_up) / rampup_length, min=0.0, max=1.0
    )
...
class SemiLoss(object):
    def __call__(
        self, outputs_x, targets_x, outputs_u, targets_u, epoch, warm_up, lambda_u=25
    ):
        ...
        return Lx, Lu, linear_rampup(epoch, warm_up, lambda_u=lambda_u)
...
class NegEntropy(object):
    def __call__(self, outputs):
        outputs = outputs.clamp(
            min=1e-12
        )  # for numerical stability (https://github.com/Justherozen/ProMix/blob/40d8378193f1a098473479ebcb56dbafa89dfacb/Train_promix.py#L460)
