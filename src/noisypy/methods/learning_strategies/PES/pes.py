...
class PES(LearningStrategyModule):
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if self.current_epoch == self.warmup_epochs:
            self.model = self.noisy_refine(
                self.model, num_layer=1, refine_times=self.T2
            )
            self.model = self.noisy_refine(
                self.model, num_layer=0, refine_times=self.T3
            )
    ...
    def on_train_epoch_end(self) -> None:
        if self.current_epoch >= self.warmup_epochs:
            ...
            confident_dataset, train_criterion = update_train_data_and_criterion(
                self.model,
    ...
    def noisy_refine(self, model: Module, num_layer: int, refine_times: int) -> Module:
        ...
        model = renew_layers(
            model, last_num_layers=num_layer, model_class=self.model_type
        )
        ...
        for epoch in range(refine_times):
            ...
            for batch in tqdm(
                ...
                self.train_step(
                    move_data_to_device(batch, device), model, optimizer_refine
                )
    ...
    def train_step(
        self, batch: Any, model: Module, optimizer: Optimizer
    ) -> STEP_OUTPUT:
        x, y_noise = batch
        y_pred = model(x)
