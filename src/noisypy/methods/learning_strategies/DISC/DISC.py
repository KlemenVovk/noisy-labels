...
class DISC(LearningStrategyModule):
    ...
    def __init__(
        self,
        datamodule, classifier_cls, classifier_args, optimizer_cls, optimizer_args, scheduler_cls, scheduler_args,
        start_epoch: int,
        alpha: float, # 
        sigma: float, # 0.5
        momentum: float, # 0.95
        ...
        *args, **kwargs
        ...
        # metrics
        self.train_acc = torchmetrics.Accuracy(num_classes=C, top_k=1, task='multiclass', average="micro")
        self.val_acc = torchmetrics.Accuracy(num_classes=C, top_k=1, task='multiclass', average="micro")
        self.test_acc = torchmetrics.Accuracy(num_classes=C, top_k=1, task='multiclass', average="micro")
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch <= self.start_epoch:
            ...
            if weak_num:        
                mixup_loss = self.mixup_loss(w_imgs[weak_flag], weak_labels[weak_flag], self.model)
                mixup_loss += self.mixup_loss(s_imgs[weak_flag], weak_labels[weak_flag], self.model)
                loss_sup += mixup_loss
        ...
        self.log("train_acc", self.train_acc(w_logits, targets), on_epoch=True, on_step=False)
