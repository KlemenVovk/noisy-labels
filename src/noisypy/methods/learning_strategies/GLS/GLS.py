...
class GLS(LearningStrategyModule):
    def __init__(
        ...
        **kwargs: Any,
        ...
        if warmup_epochs == -1:
            self.criterion = lambda logits, y: loss_gls(
                logits, y, self.hparams.smooth_rate
            )
            self.stage = 1
    ...
    def on_train_epoch_end(self):
        if (
            self.current_epoch == self.hparams["warmup_epochs"] - 1
        ):  # switch to the next optimizer and scheduler
        ...
        if (
            ...
            and self.current_epoch != self.hparams["warmup_epochs"] - 1
        ):
            scheduler = (
                self.lr_schedulers()[self.stage]
                if self.hparams["warmup_epochs"] != -1
                else self.lr_schedulers()
            )
            scheduler.step()
    ...
    def configure_optimizers(self):
        ...
        optimizer_warmup = self.optimizer_cls(
            self.model.parameters(), **self.optimizer_args[0]
        )
        optimizer_main = self.optimizer_cls(
            self.model.parameters(), **self.optimizer_args[1]
        )
        scheduler_warmup = self.scheduler_cls(
            optimizer_warmup, **self.scheduler_args[0]
        )
