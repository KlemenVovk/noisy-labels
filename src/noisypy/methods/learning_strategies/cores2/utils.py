...
def loss_cores(epoch, y, t, noise_prior=None):
    ...
    for i in range(len(loss_numpy)):
        if (
            epoch <= 30
        ):  # NOTE: first 10 epochs are warmup and we use only cross entropy loss
            loss_v[i] = 1.0
...
def f_beta(epoch):
    ...
    beta3 = np.linspace(
        2, 2, num=60 + 1
    )  # +1 because lr is set before the check if the training should stop due to reaching max_epochs (so its updated at the end of each epoch, for the next epoch)
