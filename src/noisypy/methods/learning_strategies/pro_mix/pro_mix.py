...
class ProMix(LearningStrategyModule):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.bias_m = bias_m
        self.batch_size = self.datamodule.train_dataloader()[
            BATCH_MAP["train"]
        ].batch_size
        ...
        # init model
        self.model1 = ProMixModel(
            base_model_cls=classifier_cls,
            model_type=model_type,
            feat_dim=feat_dim,
            **classifier_args,
        )
        self.model2 = ProMixModel(
            base_model_cls=classifier_cls,
            model_type=model_type,
            feat_dim=feat_dim,
            **classifier_args,
    ...
    def on_train_epoch_start(self) -> None:
        if self.current_epoch >= self.warmup_epochs:
            ...
            if self.debias_beta_pl:
                self.beta = 0.1 * linear_rampup(
                    self.current_epoch, 2 * self.rampup_epochs
                )
    ...
    def eval_train(
        self, model: Module, data_loader: DataLoader, i: int, rho: float
    ) -> Tensor:
        ...
        with torch.no_grad():
            for batch in tqdm(
                data_loader[BATCH_MAP["eval_train"]],
                desc=f"Eval Train {i}",
                leave=False,
            ):
        ...
        for j in range(self.num_classes):
            ...
            if len(indices) == 0:
                continue
            bs_j = targets_list.shape[0] * (1.0 / self.num_classes)
    ...
    def label_guessing(
        ...
        if (1.0 * idx_chosen.shape[0] / batch_size) < self.threshold:
            # both networks agree
            high_conf_cond2 = (score1 > self.tau) * (score2 > self.tau) * match
            # remove already selected examples; newly selected
            high_conf_cond2 = (1.0 * high_conf_cond2 - w_x.squeeze()) > 0
    ...
    def mixup(
        ...
        lam = torch.distributions.beta.Beta(4, 4).sample().item()
        lam = max(lam, 1 - lam)
        ...
        X_w_c_mix = lam * X_w_c + (1 - lam) * X_w_c_rand
        pseudo_label_c_mix = lam * pseudo_label_c + (1 - lam) * pseudo_label_c_rand
        if use_ph:
    ...
    def loss(
        ...
        outputs_x2: Tensor,
        outputs_x_ph: Tensor,
        outputs_x2_ph: Tensor,
        idx_chosen: Tensor,
        idx_unchosen: Tensor,
        pseudo_label_l: Tensor,
        debias_px_unrel: Tensor,
        outputs_x_unrel_ph: Tensor,
        outputs_x2_unrel_ph: Tensor,
        loss_mix: Tensor,
        loss_fmix: Tensor,
        loss_mix_ph: Tensor,
        loss_fmix_ph: Tensor,
        ...
        loss_cr = self.CESoft(
            outputs_x2[idx_chosen], targets=pseudo_label_l[idx_chosen]
        ).mean()
        loss_cr_ph = self.CESoft(
            outputs_x2_ph[idx_chosen], targets=pseudo_label_l[idx_chosen]
        ).mean()

        # cross entropy loss for primary head and pseudo head
        loss_ce = self.CESoft(
            outputs_x[idx_chosen], targets=pseudo_label_l[idx_chosen]
        ).mean()
        loss_ce_ph = self.CESoft(
            outputs_x_ph[idx_chosen], targets=pseudo_label_l[idx_chosen]
        ).mean()
        ...
        beta = (
            0
            if (self.current_epoch >= 2 * self.rampup_epochs and self.beta < 1)
            else self.beta
        )
        targets_urel = ptx
        loss_unrel_ph = self.CESoft(
            outputs_x_unrel_ph[idx_unchosen], targets=targets_urel[idx_unchosen]
        ).mean()
        loss_unrel_ph += (
            self.w
            * self.CESoft(
                outputs_x2_unrel_ph[idx_unchosen], targets=targets_urel[idx_unchosen]
            ).mean()
        )
        ...
        loss_net1_ph = (
            beta * loss_unrel_ph
            + loss_ce_ph
            + self.w * (loss_cr_ph + loss_mix_ph + loss_fmix_ph)
        )
    ...
    def train_step(self, batch: Any, opt1: Optimizer, opt2: Optimizer) -> Tensor:
        ...
        outputs_x_unrel_ph = debias_output(
            outputs_x_ph, self.pi1_unrel, self.alpha_output
        )
        outputs_x2_unrel_ph = debias_output(
            outputs_x2_ph, self.pi1_unrel, self.alpha_output
        )
        outputs_a_unrel_ph = debias_output(
            outputs_a_ph, self.pi2_unrel, self.alpha_output
        )
        outputs_a2_unrel_ph = debias_output(
            outputs_a2_ph, self.pi2_unrel, self.alpha_output
        )
        ...
        with torch.no_grad():
            ...
            debias_px_unrel = debias_pl(
                outputs_x_ori, self.pi1_unrel, self.debias_beta_pl
            )
            debias_px2_unrel = debias_pl(
                outputs_a_ori, self.pi2_unrel, self.debias_beta_pl
            )
            ...
            if self.current_epoch > self.trainer.max_epochs - self.start_expand:
                ...
                hc2_sel_wx1 = self.label_guessing(
                    idx_chosen, w_x, batch_size, score1, score2, match
                )
                hc2_sel_wx2 = self.label_guessing(
                    idx_chosen_2, w_x2, batch_size, score1, score2, match
                )
        ...
        loss_mix, loss_fmix = self.mixup(
            self.model1, inputs_x, idx_chosen, pseudo_label_l, self.pi1
        )

        # mixup loss for pseudo head $h_{AP}$ of Net 1
        loss_mix_ph, loss_fmix_ph = self.mixup(
            self.model1, inputs_x, idx_chosen, pseudo_label_l, self.pi1, use_ph=True
        )
        ...
        loss_mix2, loss_fmix2 = self.mixup(
            self.model2, inputs_x, idx_chosen_2, pseudo_label_l2, self.pi2
        )

        # mixup loss for pseudo head of Net 2
        loss_mix_ph2, loss_fmix_ph2 = self.mixup(
            self.model2, inputs_x, idx_chosen_2, pseudo_label_l2, self.pi2, use_ph=True
        )
    ...
    def training_step(self, batch: Any, batch_idx: int) -> STEP_OUTPUT:
        ...
        if self.current_epoch < self.warmup_epochs:
            ...
            if batch[0].size(0) < self.batch_size:
                ...
                return
            loss, loss_net1, loss_net2, loss_net1_ph, loss_net2_ph = self.train_step(
                batch, opt1, opt2
            )
