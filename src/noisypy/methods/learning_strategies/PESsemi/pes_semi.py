...
class PES_semi(LearningStrategyModule):
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if self.current_epoch == 0:
            ...
            self.train_labels = []
            for i in tqdm(
                range(len(train_dataset)), desc="Saving Training Data", leave=False
            ):
    ...
    def on_train_epoch_end(self) -> None:
        ...
        if self.current_epoch + 1 == self.warmup_epochs:
            self.model = self.noisy_refine(
                self.model, num_layer=0, refine_times=self.T2
            )

        if self.current_epoch + 1 >= self.warmup_epochs:
            # update train dataset and criterion
            labeled_trainloader, unlabeled_trainloader, class_weights = (
                update_train_data_and_criterion(
                    self.model,
                    train_data=self.train_data,
                    noisy_targets=self.train_labels,
            ...
            update_dataloaders(
                self.datamodule, labeled_trainloader, unlabeled_trainloader
            )
    ...
    def mix_match_step(self, batch: Any, batch_idx: int) -> STEP_OUTPUT:
        ...
        mixmatch_l = (
            torch.distributions.beta.Beta(self.alpha, self.alpha).sample().item()
        )
        ...
        Lx_mean = -torch.mean(
            log_softmax(logits_x, dim=1) * mixed_target[: batch_size * 2], 0
        )
        ...
        loss = (
            Lx
            + linear_rampup(
                self.current_epoch + batch_idx / self.num_iter,
                self.warmup_epochs,
