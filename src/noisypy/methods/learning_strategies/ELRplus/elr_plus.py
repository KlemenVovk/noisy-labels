...
class ELR_plus(LearningStrategyModule):
    ...
    def update_ema_variables(
        self, model: Module, model_ema: Module, global_step: int
    ) -> None:
    ...
    def configure_optimizers(self) -> list[list[Optimizer], list[LRScheduler]]:
        optimizer1 = self.optimizer_cls(
            filter(lambda p: p.requires_grad, self.model1.parameters()),
            **self.optimizer_args,
        )
        optimizer2 = self.optimizer_cls(
            filter(lambda p: p.requires_grad, self.model2.parameters()),
            **self.optimizer_args,
        )
